...
records =list(SeqIO.parse("dm6_PacBioSim_27species_shortintron.fa", "fasta"))
...
while(index_num<len(records)-1):
    UTR_id=re.findall(r"chr.+",records[index_num].id)[0]
    while(index_num<len(records)-1 and UTR_id==re.findall(r"chr.+",records[index_num].id)[0]):
        index_num+=1
...
uORF_matrix.to_csv("ATG_matrix_dm6_PacBioSim_27species_shortintron.csv")
